...
function validatePhone(phoneE) {
  return /^\+?([0-9]{2})\)?[-. ]?([0-9]{4})[-. ]?([0-9]{4})$/.test(phoneE)
}
...
btnRegisterEl.addEventListener('click', function(){
  ...
  if (_.isEmpty(fullnameEl)){
    ...
    document.querySelector('.fullname-error').setCustomValidity("Vui lòng nhập tên dưới 2 kí tự")
  } else if (fullnameEl.trim().length > 50){
    document.querySelector('.fullname-error').setCustomValidity("Vui lòng nhập tên trên 50 kí tự")
  }
  else {
