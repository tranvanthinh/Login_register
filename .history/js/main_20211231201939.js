...
btnRegisterEl.addEventListener('click', function(){
  ...
  } else if(!validatePhone(phoneEl)) {
    document.querySelector('.phone-error').setCustomValidity("Không được nhập bằng kí tự")
  }
  ...
  if (_.isEmpty(fullnameEl)){
    ...
    document.querySelector('.fullname-error').setCustomValidity("Vui lòng nhập tên trên 2 kí tự")
  } else if (fullnameEl.trim().length > 50){
    document.querySelector('.fullname-error').setCustomValidity("Vui lòng nhập tên dưới 50 kí tự")
  }
  else {
...
// Đăng nhập
let btnLoginEl = document.querySelector('.btnEl')
btnLoginEl.addEventListener('click', function(){
  let email = document.querySelector('#emailEl').value
  let password = document.querySelector('#passwordEl').value

  // Xử lý cho Email
  if (_.isEmpty(email)){
    document.querySelector('.emailEl-error').setCustomValidity("Vui lòng nhập đủ thông tin")
  } else if (!validateEmail(email)){
    document.querySelector('.emailEl-error').setCustomValidity("Vui lòng nhập email đúng định dạng")
  }
  else {
    document.querySelector('.emailEl-error').setCustomValidity("")
  }

  // Xử lý cho mật khẩu
  if (_.isEmpty(password)){
    document.querySelector('.passwordEl-error').setCustomValidity("Vui lòng nhập đủ thông tin")
  } else {
    document.querySelector('.passwordEl-error').setCustomValidity("")
  }
})